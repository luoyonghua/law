<template>
  <el-dialog
    ...
    fullscreen
  >
    <div v-loading="loading" class="latest-results-container">
      <el-empty v-if="!hasData" description="暂无数据" />
      
      <div v-else class="results-container-split">
        <!-- 左侧：原始文件预览 -->
              ...
              <div class="content-inner">
                ...
                <div v-if="reviewData">
                  <el-card shadow="never" class="info-card">
                    ...
                    <el-descriptions :column="2" border>
                      <el-descriptions-item label="文件名称">
                        {{ reviewData.file_name }}
                      </el-descriptions-item>
                      <el-descriptions-item label="文书类型">
                        <el-tag>{{ reviewData.doc_type }}</el-tag>
                      </el-descriptions-item>
                      <el-descriptions-item label="审查方式">
                        <el-tag :type="reviewData.review_type === 'rule_engine' ? 'success' : 'primary'">
                          {{ reviewData.review_type === 'rule_engine' ? '规则库审查' : '逻辑审查' }}
                        </el-tag>
                      </el-descriptions-item>
                      <el-descriptions-item label="审查状态">
                        <el-tag :type="reviewData.status === 'completed' ? 'success' : 'info'">
                          {{ reviewData.status === 'completed' ? '已完成' : '进行中' }}
                        </el-tag>
                      </el-descriptions-item>
                  ...
                  <el-card shadow="never" class="info-card">
                    ...
                    <el-row :gutter="16">
                      <el-col :span="6">
                        <el-statistic title="合规评分" :value="reviewData.compliance_score" suffix="%" />
                      </el-col>
                      <el-col :span="6">
                        <el-statistic title="问题总数" :value="reviewData.total_issues" />
                      </el-col>
                      <el-col :span="4">
                        <el-statistic 
                          title="严重问题" 
                          :value="reviewData.high_issues"
                      ...
                      <el-col :span="4">
                        <el-statistic 
                          title="中等问题" 
                          :value="reviewData.medium_issues"
                  ...
                  <!-- 规则检查结果 -->
                  <el-card v-if="reviewData.checked_rules && reviewData.checked_rules.length > 0" shadow="never" class="info-card">
                    <template #header>
                      <div class="card-header-with-action">
                        <span class="card-title">规则检查结果 ({{ reviewData.checked_rules.length }})</span>
                    ...
                    <div class="rules-list">
                      <div
                        v-for="(rule, index) in reviewData.checked_rules"
                        :key="index"
                        class="rule-item"
                  ...
                  <el-card shadow="never" class="info-card">
                    <template #header>
                      <span class="card-title">问题列表 ({{ reviewData.issues.length }})</span>
                    </template>
                    <el-empty v-if="reviewData.issues.length === 0" description="未发现问题" />
                    <div v-else class="issues-list">
                      <div
                        v-for="(issue, index) in reviewData.issues"
                        :key="index"
                        class="issue-item"
...
interface Props {
  visible: boolean
  documentInfo?: Api.Documents.DocumentInfo
}
...
const reviewData = ref<Api.Documents.ReviewDetailRecord>()
...
const isPdf = computed(() => {
  if (!props.documentInfo) return false
  return props.documentInfo.file_name.toLowerCase().endsWith('.pdf')
})
...
const passedRulesCount = computed(() => {
  if (!reviewData.value?.checked_rules) return 0
  return reviewData.value.checked_rules.filter(r => r.status === 'passed').length
})

const failedRulesCount = computed(() => {
  if (!reviewData.value?.checked_rules) return 0
  return reviewData.value.checked_rules.filter(r => r.status === 'failed').length
})
...
const parsedRecommendations = computed(() => {
  ...
  if (reviewData.value.recommendations && Array.isArray(reviewData.value.recommendations)) {
    return reviewData.value.recommendations
  }
  
  // 否则尝试解析 suggestions 字段
  if (!reviewData.value.suggestions || reviewData.value.suggestions.length === 0) {
    return []
  }
  
  const recommendations: Api.Documents.ReviewRecommendation[] = []
  
  for (const suggestion of reviewData.value.suggestions) {
    // 如果已经是对象，直接使用
...
const loadLatestResults = async () => {
  ...
  try {
    // 获取审查历史列表（最新1条）
    const historyData = await fetchDocumentReviewHistory(props.documentInfo.doc_id, 1)
    
    if (historyData.reviews && historyData.reviews.length > 0) {
      // 获取最新审查记录的详情
      const latestReview = historyData.reviews[0]
      const reviewDetail = await fetchReviewDetail(latestReview.review_id)
      reviewData.value = reviewDetail
    } else {
      ElMessage.info('暂无审查历史记录')
    }
...
const loadDocxPreview = async () => {
  if (!props.documentInfo || !docxPreviewContainer.value) return
...
const handleExport = (format: 'docx' | 'pdf') => {
  if (!reviewData.value || !props.documentInfo) return

  const url = generateReviewReport(props.documentInfo.doc_id, {
    format,
    review_id: reviewData.value.review_id
  })
  
  const link = document.createElement('a')
  link.href = url
  link.download = `审查报告_${props.documentInfo.file_name}.${format}`
