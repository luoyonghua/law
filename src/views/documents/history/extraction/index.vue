<template>
  <div class="extraction-history-container">
    ...
    >
      <!-- 多文档 Tab 切换 -->
      <el-tabs v-if="displayRecords.length > 1" v-model="activeRecordIndex" type="card" class="record-tabs" @tab-change="handleTabChange">
        <el-tab-pane
...
const displayRecords = ref<Api.Documents.ExtractionHistoryRecord[]>([])
...
const dialogTitle = computed(() => {
  if (displayRecords.value.length > 1) {
    return `提取详情 (共 ${displayRecords.value.length} 个文档)`
  }
  return '提取详情'
})
...
// 查看多个详情（批量提取时使用）
const handleViewMultipleDetails = async (recordsToView: Api.Documents.ExtractionHistoryRecord[]) => {
  if (recordsToView.length === 0) return
  
  displayRecords.value = recordsToView
  activeRecordIndex.value = '0'
  currentRecord.value = recordsToView[0]
  detailVisible.value = true
  
  // 如果是 docx 文件，需要加载并渲染
  if (recordsToView[0].file_name.toLowerCase().endsWith('.docx')) {
    await nextTick()
    await loadDocxPreview(recordsToView[0].doc_id)
  }
}

// Tab 切换处理
const handleTabChange = async (index: string | number) => {
  const idx = parseInt(index.toString())
  currentRecord.value = displayRecords.value[idx]
  
  // 清除之前的高亮
  clearHighlight()
  
  // 如果是 docx 文件，需要重新加载预览
  if (currentRecord.value.file_name.toLowerCase().endsWith('.docx')) {
    await nextTick()
    await loadDocxPreview(currentRecord.value.doc_id)
  }
}
...
const checkAndAutoOpen = async () => {
  const { autoOpen, count } = route.query
  
  if (autoOpen === 'latest' && records.value.length > 0) {
    await nextTick()
    
    const recordCount = parseInt(count as string) || 1
    
    if (recordCount > 1) {
      // 批量提取：获取前 N 条记录并全部打开
      const recordsToOpen = records.value.slice(0, Math.min(recordCount, records.value.length))
      
      ElMessage.success({
        message: `批量提取完成！共 ${recordCount} 个文档，已为您打开 ${recordsToOpen.length} 个文档`,
        duration: 3000
      })
      
      // 打开多个记录
      await handleViewMultipleDetails(recordsToOpen)
    } else {
      // 单个提取：只打开第一条记录
      await handleViewDetail(records.value[0])
    }
  }
}
...
onMounted(async () => {
  ...
  await checkAndAutoOpen()
})

// 监听路由变化，支持从其他页面跳转过来自动打开
watch(() => [route.query.autoOpen, route.query.refresh], async ([newAutoOpen, newRefresh]) => {
  // 如果有 refresh 参数，重新获取数据
  if (newRefresh) {
    await getHistoryRecords()
  }
